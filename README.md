...
===
# Overview
This repository presents a reproduced benchmark for the [DockerizeMe](https://github.com/dockerizeme/dockerizeme/) repository, together with analysis tools, automation scripts, and patches to make the setup runnable for Docker 28.1.1 and Python 3.11.

The intention of this repository is to evaluate how well the [Gistable](github.com/gistable/gistable) dataset may be applicable to the use case of Automatic Program Repair (APR) after Breaking Changes (BCs) from Python dependency updates.

# Additions to Dockerizeme

## Relevant Information regarding DockerizeMe
The original repository is now nearly 7 years old. It also uses Python 2, which by now is not supported anymore, so we **updated the [Python versions](src/languages/python/)** to Python 3 (version 3.11).

We then loaded their [`neo4j` dump](neo4j/neo4j.dump)[^1] and added the resulting Dockerfiles to this repository.

We also built & ran the docker images. The resulting logs (`build.log` for `docker build` and `run.log` for `docker run`) are added next to each snippet in the subfolders of [hard-gists](hard-gists/).

This repository also contains all the scripts that we used for generating the data.

[^1]: `neo4j` is a database server, and the dump file provides data for python dependencies, enabling the identification of required modules based on lookups for import statements.

### Additional Remarks
If you do not want to clone generated data, check out which branches are available in this repository.

Original repository: [DockerizeMe](https://github.com/dockerizeme/dockerizeme/).
...
Then, start the `neo4j` database server, without authentication:
```bash
...
> [!NOTE]
> We had to adjust this command too, probably the original repository's README had forgotten some syntax. The original command was:
> ```bash
> docker stop dockerizeme-neo4j
...
> [!NOTE]
> We suggested `dockerizeme-neo4j` instead of `neo4j` as name so you don't override your `neo4j` instance. If there are problems because we did not consistently use the new name, just delete the `dockerizeme-` prefixes everywhere.
...
docker stop dockerizeme-neo4j
# docker run --rm -it -v "$(pwd)/neo4j:/data" dockerizeme-neo4j neo4j-admin load --force --from=/data/<filename>
# Since we got an error indicating another API usage, we had to patch it, inserting "database" and changing to "from-path":
docker run --rm -it -v "$(pwd)/neo4j:/data" dockerizeme-neo4j neo4j-admin database load --overwrite-destination --from-path=/data dockerizeme-neo4j
docker start dockerizeme-neo4j
```
...
However, this will just print both Node.js and Docker Build output to the console. We recommend this command:
...
To automate the generation of all Dockerfiles, we created the [generate-all-dockerfiles.sh](generate-all-dockerfiles.sh) script.
> [!NOTE]
...
To automate the building and running, we added the [run_and_log_docker_images.sh](./run_and_log_docker_images.sh) script. 
> [!IMPORTANT]
> The docker images will increase to large sizes (in total, several hundreds of GB). We currently just avoid this by enforcing that 5GB of disk space have to be free, and clean the images when the limit is hit. We aim to do this automatically in the future, you are free to adjust our script; if you are lazy, just adjust the `5` to how much GB you want to keep free on your disk in this line [(see line 19)](./run_and_log_docker_images.sh):
> ```bash
...
## Analyze the results
We provided [`summarize_logs.py`](./summarize_logs.py), which creates [`summary_build.csv`](./hard-gists/summary_build.csv) and [`summary_run.csv`](./hard-gists/summary_run.csv).

Also, We provided a [plotting script](./plot_summary.py) for those summary results, which requires `pandas` and `matplotlib` pip dependencies to be installed.

We recommend installing pip dependencies via venv:
```bash
python -m venv .venv
...
Also, we provided a [`fix_dockerfile.sh`](./fix_dockerfile.sh) script that fixes the Dockerfiles in case you forgot to remove the Node.js output (lines starting with "`>`" that lead to errors).
...
[4] "Gistable":
- **Code:** https://github.com/gistable/gistable
- **Paper:** E. Horton and C. Parnin, "Gistable: Evaluating the Executability of Python Code Snippets on GitHub," 2018 IEEE International Conference on Software Maintenance and Evolution (ICSME), Madrid, Spain, 2018, pp. 217-227, doi: 10.1109/ICSME.2018.00031. https://ieeexplore.ieee.org/document/8530031